(function () {
  ...
  function isChild (node, parentClass) {
    ...
    if (isNode) {
      if (Array.isArray(parentClass)) {
        ...
        return !!node.closest(parentClass)
      }
    }
  }
  ...
  (function () {
    ...
    if (links) {
      Array.from(links).forEach(function (link) {
        ...
        isExternal = !!((url && typeof url === 'string' && url.startsWith('http')) && !containsClass(link, 'nav_item') && !isChild(link, ['.archive', '.article', '.post_nav', '.pager']))
        if (isExternal) {
          target = 'target'
          rel = 'rel'
          blank = '_blank'
  ...
  let headingNodes = []; let results; let link; let icon; let current; let id
